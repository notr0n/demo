...
export default function Russiatwo() {
    const classes = useStyles();
    const [age, setAge] = React.useState('');
    ...
    return ( < Blur img = {
        ...
        enableStyles className = {
            classes.bg
        } >
        <
        center >
        <p className = {classes.country} > روسیه(الکترونیک) </p>
        ...
        <MenuItem value = ""className = {classes.menuitem} > < em > فقط سنت پترزبورگ، گالینگراد لنینگراد، مدت اقامت هشت روز </em> </MenuItem >
        </Select>
        </FormControl>
        </center>
        </Blur>
...
function china1() {return ( <div > Hi </div>);}

function china2() {return ( <div > Hello </div>);}